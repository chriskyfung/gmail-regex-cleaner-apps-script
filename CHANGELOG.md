...
- Improved workflow efficiency with caching and concurrency.
- Refined Dependabot configuration for dependency updates.

### Changed

- Bump `rollup` from `4.53.3` to `4.55.1` in the rollup group (#61, #63).
- Bump `eslint`, `eslint-plugin-jest`, `@eslint/js` and `globals` in the eslint group (#60, #62, #66).
- Bump `prettier` in the development-dependencies group (#55, #59).
- Bump `qs` from `6.14.0` to `6.14.1` (#65).
- Bump `jws` from `4.0.0` to `4.0.1` (#58).
- Bump `@modelcontextprotocol/sdk` from `1.21.0` to `1.24.0` (#57).
- Bump `express` from `5.1.0` to `5.2.1` (#56).

### Documentation

- Enhanced README with project badges and section emojis.
